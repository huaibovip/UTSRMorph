...
DEFAULT_INPUT_PATH = Path(
    "/home/zrs/CTMRI/OASIS_L2R_2021_task03/Submit/submission/")
DEFAULT_GROUND_TRUTH_PATH = Path(
    "/home/zrs/CTMRI/OASIS_L2R_2021_task03/Test_nii/")
DEFAULT_GROUND_CSV_PATH = Path("/home/zrs/CTMRI/OASIS_L2R_2021_task03/")
DEFAULT_EVALUATION_OUTPUT_FILE_PATH = Path(
    "/home/zrs/CTMRI/OASIS_L2R_2021_task03/Output/metrics_trans_over.json")
...
def raise_missing_file_error(fname):
    message = (f"The displacement field {fname} is missing. "
...
class EvalVal():
    ...
    def load_task_03(self):
        self.pairs_task_03 = self.load_pairs(DEFAULT_GROUND_CSV_PATH /
                                             'pairs_val.csv')
        self.segs_task_03 = self.load_segs_task_03()
        self.disp_fields_task_03 = self.load_disp_fields(
            self.pairs_task_03, DEFAULT_INPUT_PATH / 'task_03',
            np.array([3, 80, 96, 112]))
    ...
    def load_segs_task_03(self):
        ...
        for i in np.unique(indices):
            case = self.nifti_loader.load(fname=DEFAULT_GROUND_TRUTH_PATH /
                                          'seg{:04d}.nii.gz'.format(i))
    ...
    def merge_ground_truth_and_predictions_task_03(self):
        ...
        for _, row in self.pairs_task_03.iterrows():
            case = {
                'seg_fixed':
                self.segs_task_03.loc[row['fixed']],
                'seg_moving':
                self.segs_task_03.loc[row['moving']],
                'disp_field':
                self.disp_fields_task_03.loc[(row['fixed'], row['moving'])]
            }
            cases += [case]
    ...
    def score_task_03(self):
        ...
        for idx, case in self.cases_task_03.iterrows():
            self.cases_results_task_03 = self.cases_results_task_03.append(
                self.score_case_task_03(idx=idx, case=case), ignore_index=True)
    ...
    def score_case_task_03(self, *, idx, case):
        ...
        disp_field = self.numpy_loader.load_image(disp_field_path).astype(
            'float32')
        #print(disp_field.max())
        disp_field = np.array(
            [zoom(disp_field[i], 2, order=2) for i in range(3)])
        jac_det = (jacobian_determinant(disp_field[np.newaxis, :, :, :, :]) +
                   3).clip(0.000000001, 1000000000)
        ...
        for i in range(1, 36):
            if ((fixed == i).sum() == 0) or ((moving == i).sum() == 0):
                continue
            dice += compute_dice_coefficient((fixed == i),
                                             (moving_warped == i))
            count += 1
        dice /= count
        print(dice)
        ...
        for i in range(1, 36):
            ...
            hd95 += compute_robust_hausdorff(
                compute_surface_distances((fixed == i), (moving_warped == i),
                                          np.ones(3)), 95.)
            count += 1
        hd95 /= count
    ...
    def score_aggregates_task_03(self):
        ...
        for col in self.cases_results_task_03.columns:
            aggregate_results[col] = self.aggregate_series_task_03(
                series=self.cases_results_task_03[col])
    ...
    def load_disp_fields(self, pairs, folder, expected_shape):
        ...
        for _, row in pairs.iterrows():
            fname = folder / 'disp_{:04d}_{:04d}.npz'.format(
                row['fixed'], row['moving'])
