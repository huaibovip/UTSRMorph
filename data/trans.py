...
class Base(object):
    ...
    def __call__(self, img, dim=3, reuse=False):  # class -> func()
        ...
        if isinstance(img, Sequence):
            return [self.tf(x, k)
                    for k, x in enumerate(img)]  # img:k=0,label:k=1
...
class RandomRotion(Base):
    ...
    def sample(self, *shape):
        ...
        self.angle_buffer = np.random.randint(
            -self.angle_spectrum,
            self.angle_spectrum)  # choose the random direction
        return list(shape)
            ...
            if k == 0:
                ...
                channels = [
                    rotate(img[bs, :, :, :, c],
                           ...
                           order=0,
                           mode='constant',
                           cval=-1) for c in range(img.shape[4])
                ]
                img[bs, ...] = np.stack(channels, axis=-1)

            if k == 1:
                img[bs, ...] = rotate(img[bs, ...],
                                      self.angle_buffer,
                                      axes=self.axes_buffer,
                                      reshape=False,
                                      order=0,
    ...
    def __str__(self):
        return 'RandomRotion(axes={},Angle:{}'.format(self.axes_buffer,
                                                      self.angle_buffer)
...
class CenterCropBySize(CenterCrop):
    ...
    def sample(self, *shape):
        assert len(
            self.size) == 3  # random crop [H,W,T] from img [240,240,155]
        if not isinstance(self.size, list):
            size = list(self.size)
        else:
            size = self.size
        start = [(s - i) // 2 for i, s in zip(size, shape)]
        self.buffer = [slice(None)
                       ] + [slice(s, s + i) for i, s in zip(size, start)]
        return size
...
class RandCrop3D(CenterCrop):
    ...
    def sample(self, *shape):  # shape : [240,240,155]
        ...
        self.buffer = [slice(None)
                       ] + [slice(s, s + k) for s, k in zip(start, size)]
        return size
...
class Seg_norm(Base):
    ...
    def __init__(self, ):
        ...
        self.seg_table = np.array([
            ...
            28, 30, 31, 41, 42, 43, 44, 46, 47, 49, 50, 51, 52, 53, 54, 58, 60,
        ...
        for n in range(img.shape[0]):
            ...
            if sig > self.eps:
                if len(img.shape) == self.dim + 2:
                    ...
                    for c in range(C):
                        img[n, ...,
                            c] = ndimage.gaussian_filter(img[n, ..., c], sig)
