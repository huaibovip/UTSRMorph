...
def main():
    ...
    test_set = datasets.OASISBrainInferDataset(glob.glob(test_dir + '*.pkl'),
    ...
    with torch.no_grad():
        ...
        for data in file_names:
            ...
            x_seg, y_seg = torch.from_numpy(x_seg).cuda(), torch.from_numpy(
                y_seg).cuda()
            ...
            print(flow.shape)
            jac_det = (jacobian_determinant(flow) + 3).clip(
