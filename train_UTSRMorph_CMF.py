...
import os, utils_CMF, glob, losses
import sys
...
def main():
    ...
    save_dir = 'UTSRMorph_ncc_{}_dsc{}_diffusion_{}/'.format(weights[0], weights[1], weights[2])
    ...
    train_set = datasets.CMFDataset(glob.glob(train_dir + '*.npy'), transforms=train_composed)
    val_set = datasets.CMFInferDataset(glob.glob(val_dir + '*.npy'), transforms=val_composed)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for iii in range(15):
            for data in train_loader:
                ...
                print('Iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Reg: {:.6f}'.format(idx, 15 * len(train_loader),
                                                                                       loss.item(),
                                                                                       loss_ncc.item(),
        ...
        with torch.no_grad():
            ...
            for data in val_loader:
                ...
                distancepro = []
                for j in range(9):
                    if(movingfro[j,0] + movingfro[j,1] + movingfro[j,2] != 0 and fixfro[j,0] + fixfro[j,1] + fixfro[j,2] != 0):
        ...
        }, save_dir='experiments/'+save_dir, filename='dsc{:.4f}.pth.tar'.format(100 - np.mean(pointtre)))
