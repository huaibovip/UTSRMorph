...
import os, utils_abd, glob, losses
import sys
...
def main():
    batch_size = 1
    train_dir = r'/home/zrs/AD_xiong/xiong/train/CT/data/'
    val_dir = r'/home/zrs/AD_xiong/xiong/test/CT/data/'
    ...
    train_set = datasets.CTMRIABDDataset(glob.glob(train_dir + '*.npy'), transforms=train_composed)
    val_set = datasets.CTMRIABDInferDataset(glob.glob(val_dir + '*.npy'), transforms=val_composed)
        ...
        for ii in range(8):
            for data in train_loader:
                ...
                if (random.random() > 0.5):
                    ...
                    optimizer.step()
                    del def_seg
                print('Iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Dsc: {:.6f}, Reg: {:.6f}'.format(idx,
                                                                                                    len(train_loader) * 8,
                                                                                                    loss.item(),
                                                                                                    loss_ncc.item(),
                                                                                                    loss_dsc.item(),
