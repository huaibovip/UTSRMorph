...
from models_voxelmorph import VoxelMorph
...
def main():
    ...
    save_dir = 'VoxelMorph_mi{}_dsc{}_diffusion{}/'.format(
        weights[0], weights[1], weights[2])
    if not os.path.exists('work_dirs/voxelmorph/experiments/' + save_dir):
        os.makedirs('work_dirs/voxelmorph/experiments/' + save_dir)
    if not os.path.exists('work_dirs/voxelmorph/logs/' + save_dir):
        os.makedirs('work_dirs/voxelmorph/logs/' + save_dir)
    sys.stdout = Logger('work_dirs/voxelmorph/logs/' + save_dir)
    ...
    if cont_training:
        epoch_start = 54
        model_dir = 'work_dirs/xmorpher/experiments/' + save_dir
    ...
    best_dsc = 0
    writer = SummaryWriter(log_dir='work_dirs/voxelmorph/logs/' + save_dir)
    for epoch in range(epoch_start, max_epoch):
        ...
        save_checkpoint(
            ...
            save_dir='work_dirs/voxelmorph/experiments/' + save_dir,
